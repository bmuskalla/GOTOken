...
' Loaded from tokenizer.json by LoadTokenizer (tokenizer.bm).
...
DIM SHARED cpToByte(0 TO 323) AS LONG '   inverse of GPT-2's byte -> unicode map (BuildByteLevelMap)
...
' the GPT-2 pre-tokenizer regex: \p{L}, \p{N}, \s. The data is in unicode.bm.
